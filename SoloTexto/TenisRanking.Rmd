...
url <- 'http://www.atpworldtour.com/es/rankings/singles?rankDate=2018-04-30&rankRange=1-5000'
webpage <- read_html(url)
...
temp <- data.frame(pais = pais, puntos300418 = puntos)
temp <- docomp('sum', 'puntos300418', 'pais', data = temp)
ddd <- merge(ddd, temp, all = T)

ddd <- ddd[sort(ddd$puntos300418, decreasing = TRUE, index.return = T, na.last = T)$ix, ]
rownames(ddd) <- 1:dim(ddd)[1]
