...
url <- 'http://www.atpworldtour.com/es/rankings/singles?rankDate=2018-04-09&rankRange=1-5000'
webpage <- read_html(url)
...
temp <- data.frame(pais = pais, puntos090418 = puntos)
temp <- docomp('sum', 'puntos090418', 'pais', data = temp)
ddd <- merge(ddd, temp, all = T)

ddd <- ddd[sort(ddd$puntos090418, decreasing = TRUE, index.return = T, na.last = T)$ix, ]
